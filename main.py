...
BUBBLE_WEBHOOK_URL = os.getenv("BUBBLE_WEBHOOK_URL", "https://portalestudio7.bubbleapps.io/version-test/api/1.1/wf/receber-mensagem")

@app.post("/webhook/bubble-translate")
async def bubble_translate(payload: dict, x_api_key: str = Header(default="")):
    """
    Recebe evento do WAHA (formato WAHA) e encaminha para o Bubble
    ...
    # Limpa o phone
    phone = from_raw.replace("@c.us", "").replace("@s.whatsapp.net", "").replace("@lid", "").strip()
    if not phone:
        return {"ok": False, "reason": "no phone"}

    # Monta payload no formato Z-API
    zapi_payload: dict = {
        "phone": phone,
        "fromMe": from_me,
        "momment": timestamp,
        "type": "ReceivedCallback",
        "chatName": notify_name or phone,
        "instanceId": session,
        "messageId": msg.get("id", ""),
        "senderName": notify_name or phone,
        "participantPhone": None,
        "broadcast": False,
    }

    media = msg.get("media") or {}
    media_url = media.get("url", "") if isinstance(media, dict) else ""

    if msg_type == "text":
        zapi_payload["text"] = {"message": body}
    elif msg_type in ("image", "sticker"):
        zapi_payload["image"] = {
            "imageUrl": media_url,
            "caption": body or "",
            "mimeType": media.get("mimetype", "image/jpeg") if isinstance(media, dict) else "image/jpeg"
        }
        ...
        zapi_payload["document"] = {
            "documentUrl": media_url,
            "fileName": media.get("filename", "arquivo") if isinstance(media, dict) else "arquivo",
            "caption": body or ""
        }
    elif msg_type == "video":
        zapi_payload["video"] = {
        ...
        zapi_payload["text"] = {"message": body or f"[{msg_type}]"}

    # Encaminha para o Bubble
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.post(BUBBLE_WEBHOOK_URL, json=zapi_payload)
            print(f"[BUBBLE-TRANSLATE] {msg_type} from {phone} → Bubble {r.status_code}")
            return {"ok": True, "bubble_status": r.status_code, "type": msg_type}
    except Exception as e:
        print(f"[BUBBLE-TRANSLATE] erro: {e}")
